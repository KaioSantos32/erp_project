...
import 'package:erp/functions/funcoes_mongo/entrar_xml_no_banco.dart';
...
void lerXml(tipoNota, context) async {
  ...
  if (arquivoSelecionado != null) {
    ...
    // Essa parte do código deve verificar se a nota que foi selecionada já foi dada entrada
    // incompleto, ainda não funciona, mas não da entrada novamente no banco de dados por conta da chave da NF
    var nota = mongoPesquisaNota(numeroNota: numeroNf, tipoNota: tipoNota);
    verificaNoDb(nota);
    ...
    if (tipoNota == 1) {
      ...
      if (chaveNotaFiscal.contains(empresaCnpj)) {
        ...
        for (var produto in document.xpath("/nfeProc/NFe/infNFe/det")) {
          var valorProduto =
              produto.findAllElements("vUnCom").single.innerText.toString();
        ...
        file.rename(
            "${oldPath.substring(0, indiceDoNumeroDaNf)}IMPORTADO-$numeroDaNf");
      } else {
        showDialog(
          context: context,
