...
Future<List> mongoPesquisaNota({
  ...
  try {
    if (tipoPesquisa == 01 ||
      ...
      notas = coll.find(
        where.eq("dhEmissao", {"\$gte": "$dataInicialFormatada"}).sortBy(
            "dhEmissao",
            descending: true),
      );
      ...
      notas = coll.find(where
          .eq("dhEmissao", {"\$gte": "$dataInicialFormatada"}).eq("produtos", {
        "\$elemMatch": {
          "nome_produto": {"\$regex": nomeProduto!.toUpperCase()}
        }
      }).sortBy("dhEmissao", descending: true));
      ...
      notas = coll.find(
        where.eq("dhEmissao", {"\$lte": "$dataFinalFormatada"}).sortBy(
            "dhEmissao",
            descending: true),
      );
      ...
      notas = coll.find(where
          .eq("dhEmissao", {"\$lte": "$dataFinalFormatada"}).eq("produtos", {
        "\$elemMatch": {
          "nome_produto": {"\$regex": nomeProduto!.toUpperCase()}
        }
      }).sortBy("dhEmissao", descending: true));
      ...
      notas = coll.find(where
          .eq("dhEmissao", {"\$lte": "$dataFinalFormatada"}).eq(
              "$tipoEmpresa.nome",
              {"\$regex": "^${nomeFornecedor!.toUpperCase()}"}).eq("produtos", {
        "\$elemMatch": {
          "nome_produto": {"\$regex": nomeProduto!.toUpperCase()}
        }
      }).sortBy("dhEmissao", descending: true));
      ...
      notas = coll.find(
        where.eq("nf", numeroNota).eq("$tipoEmpresa.cnpj", cnpj),
      );
    } else {
      notas = coll.find(
        where
...
excluirCadastroEntidade(cnpj, tipoEntidade, nomeEmpresa, context) async {
  ...
  ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(
      content: Text(
        'Cadastro da empresa $nomeEmpresa foi excluida',
